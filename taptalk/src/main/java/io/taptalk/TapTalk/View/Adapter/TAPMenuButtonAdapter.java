...
public class TAPMenuButtonAdapter extends TAPBaseAdapter<TAPMenuItem, TAPBaseViewHolder<TAPMenuItem>> {
    ...
    private TAPProfileActivity.ProfileMenuInterface menuInterface;

    public TAPMenuButtonAdapter(List<TAPMenuItem> items, TAPProfileActivity.ProfileMenuInterface menuInterface) {
        setItems(items, true);
                    ...
                    private ValueAnimator getTransitionGreen() {
                        if (null == transitionToGreen) {
                            transitionToGreen = ValueAnimator.ofArgb(
                                    ...
                                    itemView.getContext().getResources().getColor(R.color.tap_greenBlue));
                            transitionToGreen.setDuration(DEFAULT_ANIMATION_TIME);
                            transitionToGreen.addUpdateListener(valueAnimator -> ivMenuIcon.setColorFilter(
