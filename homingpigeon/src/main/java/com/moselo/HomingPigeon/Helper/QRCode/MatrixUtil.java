...
package com.moselo.HomingPigeon.Helper.QRCode;
...
final class MatrixUtil {
  ...
  // Set all cells to -1.  -1 means that the cell_chat_product_item_user is empty (not set yet).
  //
  ...
  static void embedDataBits(BitArray dataBits, int maskPattern, ByteMatrix matrix)
    ...
    int bitIndex = 0;
    int direction = -1;
    // Start from the right bottom cell_chat_product_item_user.
    ...
    while (x > 0) {
      ...
      while (y >= 0 && y < matrix.getHeight()) {
        for (int i = 0; i < 2; ++i) {
          int xx = x - i;
          // Skip the cell_chat_product_item_user if it's not empty.
      ...
      if (y >= 0) {
        for (int x : coordinates) {
          if (x >= 0 && isEmpty(matrix.get(x, y))) {
            // If the cell_chat_product_item_user is unset, we embed the position adjustment pattern here.
