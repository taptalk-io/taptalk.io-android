...
final class MatrixUtil {
  ...
  // Set all cells to -1.  -1 means that the tap_cell_chat_product_item is empty (not set yet).
  //
  ...
  static void embedDataBits(BitArray dataBits, int maskPattern, ByteMatrix matrix)
    ...
    int bitIndex = 0;
    int direction = -1;
    // Start from the right bottom tap_cell_chat_product_item.
    ...
    while (x > 0) {
      ...
      while (y >= 0 && y < matrix.getHeight()) {
        for (int i = 0; i < 2; ++i) {
          int xx = x - i;
          // Skip the tap_cell_chat_product_item if it's not empty.
      ...
      if (y >= 0) {
        for (int x : coordinates) {
          if (x >= 0 && isEmpty(matrix.get(x, y))) {
            // If the tap_cell_chat_product_item is unset, we embed the position adjustment pattern here.
